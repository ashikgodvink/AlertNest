  ...
  return (
    <div style={{ background: 'var(--bg-dark)', borderRadius: '12px', padding: '20px 22px', border: '1px solid var(--border)', boxShadow: '0 2px 8px rgba(0,0,0,0.2)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
        <span style={{ fontSize: '13px', fontWeight: '600', color: 'var(--text)' }}>Activity</span>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '11px' }}>
        {items.length === 0 && <p style={{ fontSize: '12px', color: 'var(--muted)', margin: 0 }}>No recent activity</p>}
        {items.map((item, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '9px' }}>
            ...
            <p style={{ margin: 0, fontSize: '12px', color: 'var(--muted)', lineHeight: '1.6' }}>{item.text}</p>
          </div>
        ))}
      </div>
    </div>
